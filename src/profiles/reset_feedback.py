...
def main() -> None:
    ...
    print(f"{'channel':22} {'thr':>6} {'appr':>5} {'rej':>5}   ->  reset")
    print("-" * 60)
    ...
    print("-" * 60)
    if apply:
        print(f"Reset {changed} profile(s). Backups written alongside as *.json.bak.")
        print("Restart the service so the in-memory cache reloads:")
        print("  systemctl restart highlightz")
    else:
        print(f"Would reset {changed} profile(s). Re-run with --apply to write.")
